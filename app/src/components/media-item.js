...
const VideoItem = React.createClass({
  render () {
    ...
    **/
      ...
      console.log(-1 !== this.props.videoUrl.indexOf('taobao.com')?"taobaoA":"notaobao");
  ...
  loadTaobaoVideo(vid,videoUrl,width,height,posterUrl){
    console.log("abssss"+vid+':'+width+':'+height+':'+videoUrl);
    return ()=>{

      tb_player_object.embedPlayer(
      qTJSON(vid,width,height,videoUrl),
      {autoplay:"true",poster:posterUrl},
...
const MediaItem = React.createClass({
  render () {
     ...
     **/
    var factors = this.props.aspectRatio.split(':');
    var width = 1;
    var height = 1;
    if (this.props.width) {
      if (factors[1] === '-1') {
